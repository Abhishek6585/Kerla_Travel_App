  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-sm text-gray-600">Loading India Travel Tracker...</p>
      </div>
    </div>
  );
}
...
function AppMain() {
  ...
  const handleSignIn = async (email: string, password: string) => {
    try {
      ...
      toast.success('Welcome back to India Travel Tracker! 🇮🇳');
    } catch (error: any) {
  ...
  const handleSignUp = async (email: string, password: string, name: string) => {
    try {
      ...
      toast.success('Welcome to India Travel Tracker! Your account has been created. 🇮🇳');
    } catch (error: any) {
    ...
    return (
      <AppContent
        user={{
          ...
          city: user.location?.state || 'India',
